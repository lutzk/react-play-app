...
class ApiClient {
  ...
  constructor(_incomingReq = false) {
    this.incomingReq = _incomingReq;
    this.verbs.map(verb => // eslint-disable-line
  ...
  client(verb) {
    return async (path: any, { params, data, headers }: any = {}) => {
      ...
      try {
        request = superagent[verb](formatUrl(path));
        request = this.setReqData({ request, incomingReq, headers, params, data });
        result = await request;
  ...
  setReqData({ request, incomingReq, headers, params, data }: any = {}) {
